...
export function VolumeAnalytics({ status, points }: { status: ControlStatus; points: NiftyVolumePoint[] }) {
  ...
  const summary = summarizeVolumeSession(currentRun);
  const latest = currentRun.at(-1);
  return <section className="card terminal-section volume-analytics-card">
    <div className="section-heading compact"><div><p className="eyebrow">Volume / participation</p><h2>Current run activity profile</h2></div><span>Resets after a missing minute</span></div>
    <div className="volume-analytics-grid">
      ...
      <div><span>Average minute</span><strong>{summary.average === null ? "—" : formatIndianVolume(summary.average)}</strong><small>current run average</small></div>
      <div><span>Run-relative RVOL</span><strong>{summary.relative === null ? "—" : `${summary.relative.toFixed(2)}x`}</strong><small>not historical RVOL</small></div>
      <div><span>Current run volume</span><strong>{summary.cumulative === null ? "—" : formatIndianVolume(summary.cumulative)}</strong><small>aggregate shares</small></div>
      <div><span>Current run turnover</span><strong>{turnover(summary.cumulativeTurnover)}</strong><small>constituent turnover</small></div>
