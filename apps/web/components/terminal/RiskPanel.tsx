...
export function RiskPanel({ status, refresh }: { status: ControlStatus; refresh: () => Promise<void> }) {
  ...
  return <>
    ...
    <section className="dashboard-grid terminal-section">
      <article className="card span-7"><div className="section-heading compact"><div><p className="eyebrow">Risk controls</p><h2>Global limits</h2></div><span>Read-only web access</span></div><BackendUnavailable title="Risk-limit editing is not exposed" description="Active DB-backed strategy and risk thresholds are visible on the Strategies page and reload inside Oracle. There is no authenticated update endpoint, so this page cannot safely persist edits yet." /><div className="risk-limit-grid">{["Maximum daily loss","Daily profit lock","Maximum trades / day","Trades / strategy","Concurrent positions","Capital exposure","Risk / trade","Consecutive-loss cutoff","Maximum quantity","Maximum premium","Trading window"].map((label) => <div key={label}><span>{label}</span><strong>See Strategies</strong></div>)}</div></article>
