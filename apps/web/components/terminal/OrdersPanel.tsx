...
export function OrdersPanel({ status }: { status: ControlStatus }) {
  ...
  const orders = useMemo(() => status.paperOrders.filter((order) => {
    ...
    return true;
  }), [date, instrument, side, status.paperOrders, view]);
  ...
  return <>
    <section className="terminal-section card">
      ...
      <div className="filter-bar"><label><span>Date</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label><span>Instrument</span><input placeholder="Search symbol" value={instrument} onChange={(event) => setInstrument(event.target.value)} /></label><label><span>Side</span><select value={side} onChange={(event) => setSide(event.target.value)}><option value="all">All sides</option><option value="BUY">BUY</option><option value="SELL">SELL</option></select></label><label><span>Strategy</span><select disabled><option>Level-event engine</option></select></label></div>
      {orders.length === 0 ? <EmptyState icon="orders" title="No orders match these filters" description={status.paperOrders.length ? "Adjust the lifecycle or field filters to see persisted paper orders." : "No paper order has been persisted by the execution service."} /> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Order ID</th><th>Instrument</th><th>Type</th><th>Side</th><th>Qty</th><th>Requested</th><th>Average fill</th><th>Status</th><th>Strategy</th><th>Timestamp</th><th>Slippage</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id}><td><span className="mono-id" title={order.broker_order_id ?? order.id}>{order.broker_order_id ?? `PAPER-${order.id.slice(0, 8)}`}</span></td><td><strong>{order.trading_symbol}</strong></td><td>{order.trading_symbol.endsWith("CE") ? "CE" : order.trading_symbol.endsWith("PE") ? "PE" : "Unavailable"}</td><td><span className={`side-badge ${order.side.toLowerCase()}`}>{order.side}</span></td><td className="numeric">{formatNumber(order.quantity, 0)}</td><td className="numeric">{formatNumber(order.entry_price)}</td><td className="unavailable-cell">Unavailable</td><td><span className={`status-badge ${orderView(order) === "rejected" ? "bad" : orderView(order) === "completed" ? "good" : "warn"}`}><span className={`status-dot ${orderView(order) === "rejected" ? "bad" : orderView(order) === "completed" ? "good" : "warn"}`} />{order.status}</span></td><td>Level event</td><td>{formatDateTime(order.created_at)}</td><td className="unavailable-cell">Unavailable</td></tr>)}</tbody></table></div>}
      <p className="availability-note">Requested price is the persisted paper entry premium. Average broker execution, rejection reason, and slippage are not stored by the current paper-order schema.</p>
    </section>
  </>;
}
