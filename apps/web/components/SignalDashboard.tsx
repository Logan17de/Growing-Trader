...
export default function SignalDashboard() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      if (message === "unauthorized") setAuth("guest");
      else { setNotice(message); setAuth("ready"); }
    }
  }, []);
  ...
  async function login(event: FormEvent) {
    ...
    } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Login failed"); }
    finally { setBusy(""); }
  }
  ...
  async function saveLevel(event: FormEvent) {
    event.preventDefault(); setBusy("level"); setNotice("");
    try {
      ...
      setLevelName(""); setLevelPrice(""); setNotice("Support/resistance level saved."); await loadStatus();
    } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Could not save level"); }
    finally { setBusy(""); }
  }
  ...
  async function removeLevel(id: string) {
    ...
    try {
      ...
      setNotice("Trading level removed."); await loadStatus();
    } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Could not remove level"); }
  ...
  const currentExposure = paper.open_paper_position?.entry_price && paper.open_paper_position.quantity ? paper.open_paper_position.entry_price * paper.open_paper_position.quantity : null;
  ...
  return <TerminalShell activeRoute="dashboard" status={status} onLogout={logout}>
    <section className="page-hero terminal-page-hero"><div><p className="eyebrow">Command center · Paper execution</p><h1>Trading operations</h1><p className="muted">A real-data view across market state, paper strategy execution, risk decisions, orders, and diagnostics.</p></div><div className="hero-actions"><span className="mode-badge"><span className="status-dot amber" />Paper mode</span><div className="hero-health"><span className={`status-dot ${status?.controlPlane.healthy ? "good" : "bad"}`} /><div><small>Control plane</small><strong>{status?.controlPlane.healthy ? "Healthy" : "Degraded"}</strong></div></div></div></section>
    ...
    <section className="dashboard-grid terminal-section">
      ...
      <article className="card span-4"><div className="section-heading compact"><div><p className="eyebrow">Market state</p><h2>NIFTY paper feed</h2></div><span className={`status-badge ${paperTone}`}><span className={`status-dot ${paperTone}`} />{paper.state ?? "Unknown"}</span></div><div className="market-state-grid"><div><span>NIFTY spot</span><strong>{formatNumber(paper.nifty_ltp)}</strong></div><div><span>NIFTY future</span><strong>{formatNumber(paper.future_ltp)}</strong><small>{paper.future_symbol ?? "Unavailable"}</small></div><div><span>Fresh constituents</span><strong>{paper.constituents_fresh ?? 0} / {paper.constituents_total ?? 50}</strong></div><div><span>Data age</span><strong>{typeof paper.data_age_seconds === "number" ? `${paper.data_age_seconds.toFixed(1)}s` : "Unavailable"}</strong></div></div><Link className="inline-link" href="/market">Open market overview <Icon name="arrow-right" /></Link></article>
    </section>

    <section className="dashboard-grid terminal-section">
      <article className="card span-7"><div className="section-heading compact"><div><p className="eyebrow">Latest signal</p><h2>Engine reasoning</h2></div><span>{status?.latestSignal ? formatDateTime(status.latestSignal.observed_at) : "No observation"}</span></div><SignalExplanation signal={signal} level={activeLevel} /></article>
      <article className="card span-5"><div className="section-heading compact"><div><p className="eyebrow">System status</p><h2>Runtime services</h2></div></div><div className="runtime-list"><div><span className={`status-icon ${oracleTone}`}><Icon name="server" /></span><div><strong>Oracle agent</strong><small>Heartbeat {formatDateTime(worker.last_heartbeat)}</small></div><span className={`status-badge ${oracleTone}`}><span className={`status-dot ${oracleTone}`} />{oracleState}</span></div><div><span className={`status-icon ${worker.groww_authenticated ? "good" : "warn"}`}><Icon name="shield" /></span><div><strong>Groww authentication</strong><small>{status?.credentials.configured ? "Encrypted credentials saved" : "Credentials missing"}</small></div><span className={`status-badge ${worker.groww_authenticated ? "good" : "warn"}`}>{worker.groww_authenticated ? "Verified" : "Pending"}</span></div><div><span className={`status-icon ${paperTone}`}><Icon name="strategy" /></span><div><strong>Paper engine</strong><small>{paper.feed_connected ? "Market feed connected" : "Market feed disconnected"}</small></div><span className={`status-badge ${paperTone}`}>{(paper.state ?? "Unknown").replaceAll("_", " ")}</span></div></div></article>
    </section>
