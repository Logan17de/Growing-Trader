...
function reportHtml(report: CurrentStatusReport, hasImage: boolean) {
  ...
  return `<!doctype html><html><body style="margin:0;background:#f3f6fa;font-family:Arial,sans-serif;color:#10243f"><div style="max-width:1120px;margin:0 auto;padding:24px">
    ...
    ${hasImage ? '<div style="margin-top:18px;background:white;border:1px solid #dfe7f1;border-radius:14px;padding:12px"><img src="cid:current-status-snapshot" alt="Growing Trader current status" style="display:block;width:100%;height:auto"></div>' : ""}
...
export async function POST(request: Request) {
  ...
  try {
    const body = await request.json().catch(() => ({})) as { imageData?: unknown; requestId?: unknown };
    const imageData = typeof body.imageData === "string" && body.imageData.startsWith("data:image/png;base64,") ? body.imageData : null;
    if (imageData && imageData.length > 6_000_000) return Response.json({ error: "snapshot image is too large" }, { status: 413 });
    ...
    if (imageData) attachments.push({ filename: `growing-trader-status-${report.sessionDate}.png`, content: imageData.slice("data:image/png;base64,".length), content_id: "current-status-snapshot", content_type: "image/png" });
    attachments.push({ filename: `growing-trader-status-${report.sessionDate}.json`, content: Buffer.from(JSON.stringify(report, null, 2)).toString("base64"), content_type: "application/json" });
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from, to: [to], subject: `📊 Growing Trader Current Status — ${report.sessionDate}`, html: reportHtml(report, Boolean(imageData)), attachments }),
      cache: "no-store",
    });
