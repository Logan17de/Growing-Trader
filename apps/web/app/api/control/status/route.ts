...
export async function GET() {
  ...
  try {
    ...
    return Response.json({
      ...
      worker: worker ? {
        ...worker,
        online: heartbeatFresh && !stopped,
        stale: heartbeat > 0 && !heartbeatFresh && !stopped,
      } : { online: false, stale: false, state: "offline" },
      latestCommand: commandResult.error ? null : commandResult.data ?? null,
      credentials: credentialResult.error ? { configured: false, updatedAt: null } : {
      ...
      paperEngine: paperResult.error ? { running: false, state: "unknown" } : {
