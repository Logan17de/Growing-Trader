...
function sanitizeOrder(value: unknown) {
  ...
  return { id: row.id, signal_id: row.signal_id ?? null, broker_order_id: row.broker_order_id ?? null,
    order_reference_id: row.order_reference_id ?? null, mode: row.mode,
...
function sanitizeTrade(value: unknown) {
  ...
  return { id: row.id, order_id: row.order_id ?? null, trading_symbol: row.trading_symbol, quantity: numeric(row.quantity), fill_price: numeric(row.fill_price),
    ...
    mode: textValue(raw.mode) ?? "paper", broker_order_id: textValue(raw.broker_order_id), order_reference_id: textValue(raw.order_reference_id) };
}
...
export async function GET() {
  ...
  try {
    ...
    const [signalsResult, ordersResult, tradesResult, outcomesResult] = await Promise.all([
      ...
      supabase.from("orders").select("id,signal_id,broker_order_id,order_reference_id,mode,trading_symbol,side,quantity,filled_quantity,average_fill_price,status,raw,created_at").order("created_at", { ascending: false }).limit(500),
