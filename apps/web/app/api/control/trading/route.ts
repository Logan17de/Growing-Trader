...
function sanitizeOrder(value: unknown) {
  ...
  return { id: row.id, signal_id: row.signal_id ?? null, broker_order_id: row.broker_order_id ?? null, mode: row.mode,
    trading_symbol: row.trading_symbol, side: row.side, quantity: numeric(row.quantity), status: row.status, created_at: row.created_at,
    entry_price: numeric(raw.entry_price), paper_fill_price: numeric(raw.paper_fill_price ?? raw.entry_price), paper_slippage: numeric(raw.paper_slippage),
...
function sanitizeTrade(value: unknown) {
  ...
  return { id: row.id, order_id: row.order_id ?? null, trading_symbol: row.trading_symbol, quantity: numeric(row.quantity), fill_price: numeric(row.fill_price),
    ...
    exit_reason: textValue(raw.exit_reason), paper_slippage: numeric(raw.paper_slippage) };
}
...
export async function GET() {
  ...
  try {
    ...
    const [signalsResult, ordersResult, tradesResult, outcomesResult] = await Promise.all([
      ...
      supabase.from("orders").select("id,signal_id,broker_order_id,mode,trading_symbol,side,quantity,status,raw,created_at").eq("mode", "paper").order("created_at", { ascending: false }).limit(500),
      supabase.from("trades").select("id,order_id,trading_symbol,quantity,fill_price,pnl,raw,executed_at").order("executed_at", { ascending: false }).limit(500),
