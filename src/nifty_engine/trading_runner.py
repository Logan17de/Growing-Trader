...
from .models import Direction, OptionContract, Signal
...
FAILED_ORDER_STATES = {"REJECTED", "FAILED", "CANCELLED"}
...
class TradingPersistence(PaperPersistence):
    ...
    def _live_orders_today(self) -> list[dict[str, Any]]:
        ...
        response = self.client.table("orders").select(
            "id,signal_id,broker_order_id,order_reference_id,trading_symbol,quantity,filled_quantity,average_fill_price,status,raw,created_at"
    ...
    def recover_submitting_entries(self, executor: GrowwOrderExecutor) -> None:
        """Resolve an uncertain LIVE BUY before the strategy is allowed to run.

        A missing/unavailable reference lookup is deliberately fatal.  Treating
        ambiguity as failure could allow a second real BUY after the first one
        was accepted by the broker but its response was lost.
        """
        for row in self._live_orders_today():
            ...
            if not reference:
                self.write_activity(
                    "critical", "live-engine", "live_entry_unresolved",
                    "LIVE entry has no broker reference", str(row.get("id")),
                    instrument=str(row.get("trading_symbol") or ""),
                )
                raise RuntimeError(f"unresolved LIVE entry {row.get('id')}: missing order reference")

            fill = executor.recover_by_reference(reference, int(row.get("quantity") or 0))
            if fill is None:
                self.write_activity(
                    "critical", "live-engine", "live_entry_unresolved",
                    "LIVE entry reference could not be reconciled", reference,
                    instrument=str(row.get("trading_symbol") or ""),
                )
                raise RuntimeError(f"unresolved LIVE entry reference {reference}; refusing to start")

            if not fill.filled:
                if fill.filled_quantity > 0:
                    raise RuntimeError(
                        f"LIVE entry {reference} has {fill.filled_quantity} filled but no reliable average price"
                    )
                if fill.status in FAILED_ORDER_STATES:
                    self.client.table("orders").update({
                        "status": fill.status,
                        "broker_order_id": fill.groww_order_id or row.get("broker_order_id"),
                        "filled_quantity": fill.filled_quantity,
                        "average_fill_price": fill.average_fill_price if fill.average_fill_price > 0 else None,
                        "raw": dict(row.get("raw") or {}) | {"live_fill": to_primitive(fill), "reconciled_after_restart": True},
                        "updated_at": datetime.now(timezone.utc).isoformat(),
                    }).eq("id", row["id"]).execute()
                    continue
                raise RuntimeError(
                    f"LIVE entry {reference} is still non-terminal ({fill.status}); refusing to start"
                )
            ...
            self.write_activity(
                "warning", "live-engine", "live_entry_recovered",
                "Recovered LIVE entry after restart", reference,
                instrument=str(row.get("trading_symbol") or ""),
            )

    def recover_pending_exits(self, executor: GrowwOrderExecutor) -> None:
        """Resolve a previously persisted LIVE SELL intent before continuing."""
        for row in self._live_orders_today():
            ...
            if fill is None:
                self.write_activity(
                    "critical", "live-engine", "live_exit_unresolved",
                    "LIVE exit reference could not be reconciled", reference,
                    instrument=str(row.get("trading_symbol") or ""),
                )
                raise RuntimeError(f"unresolved LIVE exit reference {reference}; refusing to continue")

            if not fill.filled:
                if fill.filled_quantity > 0:
                    raise RuntimeError(
                        f"LIVE exit {reference} has {fill.filled_quantity} filled but no reliable average price"
                    )
                if fill.status not in FAILED_ORDER_STATES:
                    raise RuntimeError(
                        f"LIVE exit {reference} is still non-terminal ({fill.status}); refusing to continue"
                    )
                ...
                self.write_activity(
                    "warning", "live-engine", "live_exit_rejected",
                    "Previous LIVE exit did not execute", f"{reference}: {fill.status}",
                    instrument=str(row.get("trading_symbol") or ""),
                )
                continue
            ...
            self.client.table("trades").insert({
                ...
                "raw": {
                    "side": "SELL",
                    "mode": "live",
                    "exit_reason": raw.get("pending_exit_reason") or "recovered_exit",
                    "entry_price": entry_price,
                    "broker_order_id": fill.groww_order_id,
                    "order_reference_id": fill.order_reference_id,
                    "recovered_after_restart": True,
                },
            ...
            self.write_activity(
                "warning", "live-engine", "live_exit_recovered",
                "Recovered LIVE exit after restart",
                f"{actual_exit} @ {fill.average_fill_price:.2f}",
                instrument=str(row.get("trading_symbol") or ""),
            )
    ...
    def restore_risk_state_for_mode(self, mode: str) -> tuple[RiskState, OpenPaperPosition | None]:
        ...
        if order_ids:
            response = self.client.table("trades").select(
                "order_id,pnl,executed_at"
            ).in_("order_id", order_ids).order("executed_at", desc=True).execute()
        ...
        actual_entries = [row for row in orders if str(row.get("status")) in {"OPEN", "CLOSED"}]
        last_trade_at = _parse_datetime(actual_entries[0].get("created_at")) if actual_entries else None
        open_row = next(
            (row for row in orders if str(row.get("status")) == "OPEN" and int(row.get("quantity") or 0) > 0),
            None,
        )
        ...
        state = RiskState(
            ...
            trades_today=len(actual_entries),
    ...
    def create_live_order(
        ...
        *,
        ...
        position = OpenPaperPosition(
            ...
            entry_level_name=getattr(level, "level_name", None),
        ...
        try:
            ...
            # Do not mark a submitted-or-possibly-submitted order FAILED.  The
            # SUBMITTING row plus unique DB lock forces reconciliation first.
            self.client.table("orders").update({
                "raw": raw | {"submit_error": f"{type(exc).__name__}: {exc}"},
                "updated_at": datetime.now(timezone.utc).isoformat(),
        ...
        if not fill.filled:
            ...
            if fill.filled_quantity == 0 and fill.status in FAILED_ORDER_STATES:
                update["status"] = fill.status
            else:
                update["status"] = "SUBMITTING"
            self.client.table("orders").update(update).eq("id", position.order_id).execute()
            raise RuntimeError(f"Groww LIVE entry is unresolved/not filled: {fill.status}")
        ...
        raw = raw | self._position_raw(position) | {
            "entry_price": fill.average_fill_price,
            "live_fill": to_primitive(fill),
        }
        self.client.table("orders").update({
        ...
        self.write_activity(
            "critical", "live-engine", "live_entry", "LIVE position opened",
            f"{position.quantity} × {position.trading_symbol} @ {position.entry_price:.2f}",
            instrument=position.trading_symbol,
            metadata={"broker_order_id": fill.groww_order_id},
        )
        return position
    ...
    def reduce_live_order(
        ...
        *,
        ...
        meta = self.client.table("orders").select("raw").eq("id", position.order_id).maybe_single().execute()
        stored_raw = dict((meta.data or {}).get("raw") or {})
        lot_size = int(stored_raw.get("lot_size") or 0)

        if fraction >= 0.999:
            exit_quantity = position.quantity
        else:
            if lot_size <= 0 or position.quantity % lot_size != 0:
                raise RuntimeError("partial LIVE exit blocked because lot size cannot be safely resolved")
            position_lots = position.quantity // lot_size
            exit_lots = max(1, int(math.floor(position_lots * fraction + 0.5)))
            exit_quantity = min(position.quantity, exit_lots * lot_size)

        # Timestamp first so a rejected exit can be retried with a new durable
        # reference; the reference itself is persisted before broker submission.
        reference = make_order_reference(
            "LX", f"{int(observed_at.timestamp() * 1000)}{position.order_id}"
        )
        pending_raw = stored_raw | self._position_raw(position) | {
            "mode": "live",
            "pending_exit_reference": reference,
            "pending_exit_quantity": exit_quantity,
        ...
        if not fill.filled:
            if fill.filled_quantity == 0 and fill.status in FAILED_ORDER_STATES:
                ...
                clean.pop("pending_exit_reference", None)
                clean.pop("pending_exit_quantity", None)
                clean.pop("pending_exit_reason", None)
                ...
                # Keep pending intent intact so restart/control recovery must
                # resolve the broker state before another SELL can be created.
                self.client.table("orders").update({
                    "raw": pending_raw | {"last_exit_attempt": to_primitive(fill)},
                    "updated_at": observed_at.isoformat(),
                }).eq("id", position.order_id).execute()
            raise RuntimeError(f"Groww LIVE exit is unresolved/not filled: {fill.status}")
        ...
        clean = stored_raw | self._position_raw(position) | {
            "mode": "live",
            "last_exit_fill": to_primitive(fill),
        }
        clean.pop("pending_exit_reference", None)
        clean.pop("pending_exit_quantity", None)
        clean.pop("pending_exit_reason", None)
        if remaining <= 0:
            ...
            self.write_activity(
                "critical", "live-engine", "live_exit", "LIVE position closed",
                f"{exit_reason}: P&L {pnl:.2f}", instrument=position.trading_symbol,
                metadata={"broker_order_id": fill.groww_order_id},
            )
            return pnl, True, fill

        position.quantity = remaining
        clean = clean | self._position_raw(position)
        self.client.table("orders").update({
            "quantity": remaining,
            "raw": clean,
            "updated_at": observed_at.isoformat(),
        ...
        self.write_activity(
            "warning", "live-engine", "live_partial_exit", "LIVE position reduced",
            f"Exited {actual_exit}; {remaining} remain", instrument=position.trading_symbol,
        )
...
class TradingEngineRuntime(PaperEngineRuntime):
    ...
    def _record_position_marks_mode(
        ...
        if position is None or self._execution_mode == "paper":
            return super()._record_position_marks(
                persistence, position, chain, nifty_ltp, risk_state, params, signal, options
            )
        ...
        if price is None:
            current = self._position_status(position, contract=contract)
        ...
        if manual_fraction is not None:
            pnl, closed, _fill = persistence.reduce_live_order(
                position,
        ...
        if not decision.should_exit:
            return position, None
        pnl, _closed, _fill = persistence.reduce_live_order(
            position,
            executor=self._executor,
            observed_at=now,
    ...
    def _run_session(self) -> None:
        ...
        if self._execution_mode == "live":
            ...
            self._executor = GrowwOrderExecutor(
                groww, product=str(execution.get("product") or "MIS")
            )
        ...
        self._kill_switch = bool(
            risk_control.get("kill_switch") or risk_control.get("block_new_entries")
        )
        ...
        if self._execution_mode == "live" and open_position and self._executor:
            ...
            if broker_quantity is None or broker_quantity != open_position.quantity:
                persistence.client.table("risk_control_state").update({
                    ...
                    "reason": (
                        f"LIVE position reconciliation mismatch for {open_position.trading_symbol}: "
                        f"DB={open_position.quantity}, broker={broker_quantity}"
                    ),
                    "updated_at": datetime.now(timezone.utc).isoformat(),
                }).eq("id", True).execute()
                raise RuntimeError(
                    f"LIVE position reconciliation mismatch: DB={open_position.quantity}, broker={broker_quantity}"
                )
        ...
        persistence.write_activity(
            "critical" if self._execution_mode == "live" else "info",
            component,
            "session_start",
            f"{self._execution_mode.upper()} market session started",
            f"{len(constituents)} NIFTY constituents resolved",
        )
        ...
        self._set_status(
            ...
            max_order_premium=self._max_order_premium,
            feed_connected=True,
            ...
            open_position=self._position_status(open_position),
        ...
        try:
            while not self._stop_event.is_set() and is_nse_session():
                ...
                if feed_snapshot.spot:
                    spot_age = max(
                        (datetime.now(timezone.utc) - feed_snapshot.spot.observed_at).total_seconds(),
                        0.0,
                    )
                ...
                if now_mono >= next_param_refresh:
                    ...
                    if refreshed != params:
                        ...
                        persistence.write_activity(
                            "info", "strategy", "parameters_reload",
                            "Strategy parameters reloaded", "DB-backed thresholds changed",
                        )
                ...
                if now_mono >= next_control_refresh:
                    ...
                    self._kill_switch = bool(
                        control_state.get("kill_switch") or control_state.get("block_new_entries")
                    )
                    self._close_on_kill = bool(
                        control_state.get("close_open_position_on_kill", True)
                    )
                    execution = persistence.load_execution_control()
                    self._live_armed = bool(execution["live_armed"])
                    ...
                    if self._execution_mode == "live" and self._executor:
                        ...
                        risk_state.account_equity = max(
                            available + max(risk_state.realized_pnl_today, 0.0), 1.0
                        )
                ...
                if time.monotonic() >= next_option and not self._stop_event.is_set():
                    ...
                    latest_chain = dict(
                        groww.get_option_chain(
                            exchange="NSE", underlying="NIFTY", expiry_date=expiry
                        )
                    )
                    ...
                    chain_spot = float(
                        latest_chain.get("underlying_ltp") or state.spot_price or 0.0
                    )
                    ...
                    if open_position and latest_chain:
                        open_position, last_exit_reason = self._record_position_marks_mode(
                            persistence,
                            open_position,
                            latest_chain,
                    ...
                    self._set_status(
                        last_option_refresh=observed_at.isoformat(),
                ...
                if did_quote_scan and not self._stop_event.is_set():
                    ...
                    if built is not None:
                        ...
                        try:
                            persistence.write_market_snapshot(
                                snapshot, levels, data_age, params, sectors
                            )
                        except Exception:
                        ...
                        if should_persist:
                            ...
                            if actionable:
                                persistence.write_activity(
                                    ...
                                    signal.risk.reason,
                                    instrument=(
                                        signal.contract.contract.trading_symbol
                        ...
                        if (
                            ...
                            and paper_entry_window_open(
                                signal.timestamp, params.opening_no_entry_minutes
                            )
                        ):
                            level_price = next(
                                (
                            ...
                            if self._execution_mode == "live":
                                if not self._live_armed or not self._executor:
                                    raise RuntimeError("LIVE execution became disarmed")
                                available_margin = self._executor.available_option_buy_margin()
                                open_position = persistence.create_live_order(
                                    signal_id,
                                    signal,
                                    snapshot.spot_price,
                                    level_price,
                            ...
                            open_position.trailing_activation_pct = (
                                params.exit_trailing_activation_pct
                            )
                            open_position.trailing_drawdown_pct = (
                                params.exit_trailing_drawdown_pct
                            )
                        ...
                        current_position = (
                            self.status().get("open_position") if open_position else None
                        )
                        if open_position and not current_position:
                            current_position = self._position_status(open_position)
                        available_capital = (
                            max(risk_state.account_equity - exposure, 0.0)
                            if self._execution_mode == "paper"
                            else (
                                self._executor.available_option_buy_margin()
                                if self._executor
                                else 0.0
                            )
                        )
                        self._set_status(
                            ...
                            last_signal={
                                ...
                                "reason": (
                                    "kill switch active"
                                    if self._kill_switch and signal.risk.allowed
                                    else signal.risk.reason
                                ),
                            },
            ...
            persistence.write_activity(
                ...
                component,
                "session_stop",
                f"{self._execution_mode.upper()} market session stopped",
